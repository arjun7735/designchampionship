...
if (isset($_POST['add_winner'])) {
    $stmt = $conn->prepare("INSERT INTO winners_2021 (category, level, position, unique_id, region, project_link) VALUES (?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_winner'])) {
    $stmt = $conn->prepare("UPDATE winners_2021 SET category=?, level=?, position=?, unique_id=?, region=?, project_link=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM winners_2021 WHERE id = $id");
    $_SESSION['message'] = "Winner deleted successfully!";
    header("Location: manage_winners_2021.php");
    exit;
}

// Fetch all winners
$winners = $conn->query("SELECT * FROM winners_2021 ORDER BY id ASC");
?>
...
<html>
<head>
    <title>Manage Winners 2021</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="adminwin.css">

    <script>
        // Confirm before adding a winner
        function confirmAdd() {
            return confirm("Are you sure you want to add this winner?");
        }

        // Confirm before updating a winner
        function confirmUpdate() {
            return confirm("Are you sure you want to update this winner?");
        }

        // Confirm before deleting a winner
        function confirmDelete() {
            return confirm("Are you sure you want to delete this winner?");
        }
    </script>
</head>
<body class="p-4">
    ...
    <div class="container">
        <h2 class="mb-4">Manage Winners - 2021</h2>

        <!-- Add Winner Form -->
        <form method="post" class="mb-4 border p-3 bg-light" onsubmit="return confirmAdd();">
                    ...
                    <tr>
                        <form method="post" onsubmit="return confirmUpdate();">
                            ...
                            <td class="text-nowrap">
                                ...
                                <a href="?delete=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirmDelete();">Delete</a>
                            </td>
                        </form>
                    </tr>
                <?php endwhile; ?>
