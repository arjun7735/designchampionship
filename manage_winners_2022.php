...
if (isset($_POST['add_winner'])) {
    $stmt = $conn->prepare("INSERT INTO winners_2022 (category, level, position, unique_id, region, project_link) VALUES (?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['edit_winner'])) {
    $stmt = $conn->prepare("UPDATE winners_2022 SET category=?, level=?, position=?, unique_id=?, region=?, project_link=? WHERE id=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM winners_2022 WHERE id = $id");
    $_SESSION['message'] = "Winner deleted successfully!";
    header("Location: manage_winners_2022.php");
    exit;
}

// Fetch all winners
$winners = $conn->query("SELECT * FROM winners_2022 ORDER BY id ASC");
?>

<!DOCTYPE html>
<html>
<head>
    <title>Manage Winners 2022</title>
...
<body class="p-4">
    ...
    <div class="container">
        <h2 class="mb-4">Manage Winners - 2022</h2>
